...
public class Payee implements Serializable{
	...
	public JSONObject amountValidate(BigDecimal amount) {
		if (amount.compareTo(minAmount) < 0)
			return Result.error.toJson("允许最小付款金额："+minAmount+"元！");
		
		if (amount.compareTo(maxAmount) > 0)
			return Result.error.toJson("允许最大付款金额："+maxAmount+"元！");
		
		return Result.success.toJson("校验通过！");
	}
